...
@Component({
  ...
  template: `
    <div style="max-width:520px">
      ...
      @if (loading) {
        ...
        <div class="error-box">{{ erroCarregar }}</div>
      } @else {
  ...
  styles: [`
    ...
    .avatar mat-icon { color:#d4621e; font-size:28px; width:28px; height:28px; }
...
export class PerfilComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get<Cliente>('/api/clientes/me').subscribe({
      ...
      error: (e) => {
        this.erroCarregar = e.error?.mensagem || 'Erro ao carregar perfil.';
        this.loading = false;
      }
    });
  }
  ...
  salvar() {
    ...
    this.saving = true;
    this.error = '';
    const { telefone, endereco } = this.form.getRawValue();
    this.svc.atualizar(this.clienteId, { telefone, endereco }).subscribe({
      next: () => {
        this.saving = false;
        this.snack.open('Perfil atualizado!', '', { duration: 2500 });
      },
      error: e => {
        this.error = e.error?.mensagem || 'Erro ao salvar.';
        this.saving = false;
      }
    });
  }
